...
import maids.cc.data.client.Client;
...
import maids.cc.data.rest.server.v1.api.SalesApi;
import maids.cc.data.rest.server.v1.model.OrderDetail;
import maids.cc.data.rest.server.v1.model.SaleDAO;
import maids.cc.data.sale.Sale;
import maids.cc.data.sale.SaleOrder;
import maids.cc.data.sale.SaleOrderRepository;
import maids.cc.data.sale.SaleRepository;
import maids.cc.data.sale.SaleUpdateLog;
import maids.cc.data.sale.SaleUpdateLogRepository;
		...
		for (OrderDetail saleOrderDAO : saleDAO.getOrder()) {
			...
			if (saleOrderDAO.getQuantity() > saleOrder.getQuantity()) {
				productRes = productsRepository.decreaseQuantity(product.getId(),
						saleOrderDAO.getQuantity() - saleOrder.getQuantity());
			} else {
				productsRepository.increaseQuantity(product.getId(), saleOrder.getQuantity());
				productRes = productsRepository.decreaseQuantity(product.getId(), saleOrderDAO.getQuantity());
			}
			...
			if (productRes == 1) {
				...
				saleOrderLog.setNewPrice(saleOrderDAO.getProductPrice());
				saleOrderLog.setDateCreated(Date.valueOf(LocalDate.now()));
				saleOrder.setPrice(
						saleOrderDAO.getProductPrice() == null ? product.getPrice() : saleOrderDAO.getProductPrice());
				saleOrder.setQuantity(saleOrderDAO.getQuantity());
				saleOrdersRepository.save(saleOrder);
